...
The standalone TUI control center is now a fully interactive, three-pane surface that scales to the user's actual goal state. The flat v0.6.0 goal-pane is split into a header, a live session pane (activity feed + step timeline), and a context pane (drill-down). The shell ships 11 v0.6.0 keys + 7 v0.7.0 keys + 8 navigation keys for the new help overlay.

- **Live event surfaces (A1-A5).** The server plugin now emits two append-only JSONL files in `.opencode/`: `.session-events.jsonl` (every `tool.execute.after` hook) and `.step-timeline.jsonl` (every evaluation). Two pure reader modules (`session-events.ts`, `step-timeline.ts`) are the public surface; tests pin the read/append/trim and corrupt-line-skip semantics. A UI-friendly wrapper (`templates-view.ts`) makes the existing `discoverTemplates` output work for the v0.7.0 picker. (Commit `76ab4b1`, `be47a6d`, `c0da32d`, `9c38f54`.)
- **Three-pane shell (B7-B12).** `control-center-pane.ts` is a pure layout primitive (3 layout modes: stack/stacked/compact) with a hit-test map. The shell uses `renderControlCenter` to compose the active goal pane + the live session pane. `buildSessionPane` shows the activity feed (newest first) and the step timeline (with relative time). `buildGoalPane` is extracted from `renderFrame` (behavior-preserving). The shell has a `readers` seam (`RunControlOpts.readers`) that defaults to filesystem-backed readers but tests can inject stubs. A 2s `setInterval` tick (unref'd, env-configurable via `OPENGCODE_TUI_TICK_MS`, clamped to [250ms, 30s]) refreshes the events/timeline panes. (Commits `96cf200`, `4be874b`, `664b771`, `8580e35`, `bf4ad9f`.)
- **Drill-down (C13-C17).** `control-center-history.ts` is a pure reducer (`drillReducer` + `initialDrillState`) for the drill-down state machine. The shell's drill-mode branch handles Tab (enter/switch kind), ↑/↓ (cursor), Enter (open detail / select), Esc (exit), `c` (OSC 52 copy of the current item to the clipboard), `e` (open the inline editor pre-filled with the selected steering note), `q` (universal quit). The drill overlay renders a header + the visible items (cursor-highlighted) + the full-reason detail view when open. (Commits `56fccd1`, `59c37c8`, `b21ef8d`.)
- **Picker reducer (D18).** `picker.ts` is a pure list-picker reducer (`pickReducer` + `initialPickState`) for the A/T/D modal pickers. The `down` action optionally carries an `itemCount` so the cursor can clamp at the boundary. (Commit `036f26c`.)
- **7 new actions (D19-D24).** A (view goal archive), T (view templates list), D (inline doctor), L (open .opencode/ in the OS file manager), O (alias for L), g (copy full goal state JSON to clipboard via OSC 52), Ctrl+L (redraw). The L/O actions use a `fileOpener` seam so tests inject a no-op; the default spawns the platform file manager (`explorer.exe` / `open` / `xdg-open`, detached, unref'd). The 7 actions are intercepted BEFORE the v0.6.0 dispatcher's `if (!action) return;` guard — without this, the keystrokes were silently dropped. (Commit `81be1ad`.)
- **Categorized help overlay (E25-E26).** The v0.6.0 flat help function was replaced by a categorized, paged, searchable overlay. `help-content.ts` exports the source-of-truth sections (Goal / Session / Nav) with all 18 key bindings. `help-overlay.ts` is the pure renderer: section header, key/action entries (case-insensitive substring search), pagination hint. The shell owns the `helpPage` and `helpQuery` state; the overlay opens with `?`, navigates with `n`/`p`, searches by typing, and closes with Esc/`q`. (Commits `3531aa9`, `34bd753`.)
- **Defensive guards found by the v0.7.0 adversarial probe (probe-v070.test.mjs).** `drillReducer(null, ...)` threw; `drillReducer(state, {kind: 'enter'})` on an empty list opened a "detail of nothing". Both are now defensive no-ops. The probe also covers broken `.session-events.jsonl` (mixed good/bad lines), missing `.opencode/` dir, regex-special chars in the help query, and a null byte in the search query. (Commit `f53e23a`.)
- **Cleanup.** The dead v0.6.0 `renderHelp` function was removed. (Commit `2e31136`.)
- **End-to-end walkthrough (walkthrough-v070.test.mjs).** 5 cases walk the full v0.7.0 user flow against a fake-TTY harness: empty goal → `?` → `n` → close → `Tab` → `A` → `D` → `g` → `Ctrl+L` → `q`; plus a full active-goal flow (drill-down Tab/Tab/c/Esc/L/g/q). (Commit `40ef36b`.)

**Files changed (15):** `package.json` (version bump), `CHANGELOG.md` (this section), `src/session-events.ts`, `src/step-timeline.ts`, `src/server.ts`, `src/templates-view.ts`, `src/control-center-pane.ts`, `src/control-center-logic.ts`, `src/control-center.ts`, `src/control-center-history.ts`, `src/picker.ts`, `src/help-content.ts`, `src/help-overlay.ts`, plus 13 new test files (1048 baseline tests → 1053 with the v0.7.0 additions). 1053/1053 green (3× runs, no flakes).

## 0.6.1

**Control-center polish + "no standalone web server" invariant. No behavior changes for end users; everything is a hardening pass on v0.6.0.**

- **Control center: scroll preserves the footer keybar.** `fit()` (control-center-logic.ts) now takes a `scrollOffset` and the body+footer split is anchored — the keybar stays visible while the steering list scrolls past. `renderFrame` accepts the offset, the shell wires it to `↑/↓`, and scrolling on a tall body no longer hides the actions. Test pin in `test/control-center-logic.test.mjs:107` (8 steering notes, height 8: top excludes `note-7`, scrolled includes it, both keep `[q]` visible).
- **Control center: terminal lifecycle is now testable and leak-free.** `runControlCenter` accepts an injected `onExit` hook (defaults to `process.exit`), registers `SIGINT` / `process.exit` / `stdout.resize` as named handlers, and removes every listener in `cleanupAndExit` (`stdin.off` keypress, `process.off` SIGINT, `process.off` exit, `stdout.off` resize, `stdin.pause`). The handler that re-runs `restoreTerminal` on the `process.exit` event is now a single named function (not an inline arrow) so `process.off` can find it. `stdin.resume()` is called explicitly so the keypress stream starts even when piped through a wrapping harness. Behavioral coverage in `test/control-center.test.mjs:115` (fake-TTY EventEmitter harness: raw-mode + stdin.resume asserted on entry, exit code propagated through the hook, raw mode disabled on quit).
- **TUI: alt-hotkeys wired on the dashboard, esc-literal renamed to OpenTUI's `escape`.** `src/tui.tsx` registers `alt+g` dashboard, `alt+p` pause/resume, `alt+s` steer, `alt+n` set new goal, `alt+c` clear. The Esc binding is keyed on `escape` (OpenTUI's normalized name, verified with the `@opentui/keymap/testing` harness — `h.host.press("escape")` fires the close command, `defaultPrevented=false`, `propagationStopped=false` so the host's palette-Esc still works). A new `goal.dial.set` palette entry wires the `alt+n` binding to a `setGoalFields({ condition: v })` prompt. Behavioral coverage in `test/tui-ux-fixes.test.mjs:46` (keymap harness exercises the literal binding) and `test/tui-ux-fixes.test.mjs:62` (alt+g fires the dashboard command).
- **Sidebar: footer now leads with the real key chords.** `buildSidebarFooter` swaps the legacy `"q: /goal-steer · /goal-toggle · /goal-clear · /goal-condition"` for `"keys: alt+g dashboard · alt+s steer · alt+p pause · /goal-steer"`. The `q:` prefix is gone because the sidebar is a launcher/status rail, not a "quick actions" cheat-sheet — leading with `keys:` and the actual key chords is the at-a-glance cue. The `FOOTER_MAX` truncation and the `/goal-claim` handoff-suffix are unchanged. The `q:` → `keys:` swap is pinned in `test/sidebar-logic.test.mjs:482` and the new key/hotkey assertions are in `test/sidebar-logic.test.mjs:466`.
- **README rewrite: "Desktop vs. terminal" section.** Replaces the legacy "Optional terminal dashboard / sidebar" headings with "OpenCode terminal console" / "OpenCode terminal sidebar" (matching the v0.6.0 surface naming), adds the v0.6.0 hotkey list (`alt+g/n/s/p/c`), and removes the now-stale `desktop-ui-design.md` cross-reference (replaced with a plain "this package does not ship a separate web server as a substitute for OpenCode app-side integration"). The Windows run notes added in v0.6.0 (`C:\Users\…\npm i -g opencode-autogoal`) are preserved.
- **NEW INVARIANT — "CLI surface does not ship a standalone web server command."** Pinned in `test/cli.test.mjs:120`. Asserts: `src/cli.ts` does not contain `serve [--port <n>]` help text; does not import `createControlServer` / `parseServeOptions` / `runServe`; `package.json` does not export `./control-server`; `package.json` `files` does not include `src/control-server.ts`. This is a deliberate architectural decision (the Desktop Goals panel belongs in OpenCode's `packages/app`, not as a third-party web server) and the lock-in test prevents accidental reintroduction. If a future contribution tries to re-add the web-server path, this test fails before merge.

**Files changed (11):** `package.json` (version bump), `CHANGELOG.md` (this section), `src/control-center-logic.ts` (scroll), `src/control-center.ts` (lifecycle), `src/sidebar-logic.ts` (footer), `src/sidebar.tsx` (docstring), `src/tui.tsx` (alt-hotkeys + `goal.dial.set`), `test/cli.test.mjs` (web-server invariant), `test/control-center-logic.test.mjs` (scroll pin), `test/control-center.test.mjs` (lifecycle harness), `test/sidebar-logic.test.mjs` (footer swap), `test/tui-ux-fixes.test.mjs` (escape + alt+g). Full suite 881/881 green (`npm test`).
